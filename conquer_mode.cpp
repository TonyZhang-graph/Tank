...
Conquer_Mode::Conquer_Mode(GameWindow *_ui, QGraphicsScene *_menu, QString *tanks, const QString &map_url, QObject *parent) : QGraphicsScene{parent}
{
    ...
    record = new QGraphicsPixmapItem[30];
    ...
    cooling_timer1 = new QTimer(this);
    connect(cooling_timer1, &QTimer::timeout, this, [&]() {tank[0]->cooling = false;});
    cooling_timer2 = new QTimer(this);
    connect(cooling_timer2, &QTimer::timeout, this, [&]() {tank[1]->cooling = false;});
...
void Conquer_Mode::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_Q:
        if (tank[0]->cooling == false)
        {
            cooling_timer1->start(300);
            tank[0]->cooling = true;
    ...
    case Qt::Key_U:
        if (tank[1]->cooling == false)
        {
            cooling_timer2->start(300);
            tank[1]->cooling = true;
